...
import TryAgainGraphic from '@/components/mid-fight/TryAgainGraphic';
...
const PhraseRewind: React.FC<PhraseRewindProps> = ({ onClose }) => {
  ...
  return (
    <section className="py-6 md:py-8 bg-soft-blush/30">
      <div className="bg-white rounded-lg shadow-md p-5 md:p-6 border border-mauve-rose/20">
        <div className="flex flex-col items-center mb-6 cursor-pointer" onClick={() => setIsOpen(!isOpen)}>
          ...
          <p className="text-[#22254a] text-center max-w-2xl mb-3 text-sm md:text-base">
            For next time: Discover gentler ways to express difficult feelings
          </p>
          <Button 
            variant="outline" 
            onClick={() => setIsOpen(!isOpen)}
            className="mt-2 border-[#536878] text-[#536878] hover:bg-[#536878] hover:text-white"
          >
